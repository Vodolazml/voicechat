...
```powershell
Compress-Archive -Path dist\PrivateVoiceChat\* -DestinationPath dist\PrivateVoiceChat-0.1.1-ip.zip -Force
```
...
- архив `PrivateVoiceChat-0.1.1-ip.zip`;
- логин;
- временный пароль.
...
```powershell
scp dist\PrivateVoiceChat-0.1.1-ip.zip root@72.35.246.230:/root/voicechat/downloads/
```

После перезапуска контейнера файл будет доступен по ссылке:

```text
http://72.35.246.230:8765/downloads/PrivateVoiceChat-0.1.1-ip.zip
```

Для автообновления можно прописать в `.env.ip`:

```env
VOICECHAT_CLIENT_DOWNLOAD_URL=http://72.35.246.230:8765/downloads/PrivateVoiceChat-0.1.1-ip.zip
