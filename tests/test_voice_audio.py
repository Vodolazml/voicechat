...
def test_voice_receive_frame_with_zero_local_volume_is_not_audible() -> None:
    key = b"v" * 32
    sender_id = 5
    receiver = make_client(user_id=9, key=key, volume=0)
    pcm = b"\x01\x02" * 320
    encrypted = encrypt_frame(key, pcm, make_voice_aad(11, sender_id, 0))
    packet = sender_id.to_bytes(4, "big") + encrypted
...
def test_voice_receive_frame_without_key_reports_key_problem() -> None:
    key = b"v" * 32
    sender_id = 5
    receiver = VoiceAudioClient(
        ws_url="ws://test",
        ws_headers={},
        channel_id=11,
        user_id=9,
        outgoing_media_key=key,
        media_key_for_sender=lambda sender: None,
        is_muted=lambda: False,
        is_deafened=lambda: False,
        is_locally_muted=lambda sender: False,
        local_volume=lambda sender: 100,
        noise_suppression=lambda: False,
        noise_threshold=lambda: 450,
    )
    pcm = b"\x01\x02" * 320
